...
namespace PathFinder.Common
{
    ...
    public class DAT
    {
        ...
        public DAT(Character chr, string fn)
        {
            Char = chr;
            FileName = fn;
        }
        ...
        /// <returns>BBT.</returns>
        public BBT ParseQuadTree(byte[] data, int pos, int level)
        {
        ...
        /// The vismapid
        /// </summary>
        public Dictionary<int, int> vismapid = new Dictionary<int, int>();
        ...
        private void ParseGridMesh(byte[] data, int _x, int _y, int visentryoffset, int geometryoffset)
        {
            ...
            var determ = (M2[0] * ((M2[4] * M2[8]) - (M2[5] * M2[7]))) - (M2[1] * ((M2[3] * M2[8]) - (M2[5] * M2[6]))) + (M2[2] * ((M2[3] * M2[7]) - (M2[4] * M2[6])));
            int multiBy = 1;
            ...
            for (int i = 0; i < numvert; i++)
            {
                ...
                {
                    x = (M1[0] * x + M1[4] * y + M1[8] * z + M1[12] * w),
                    y = -(M1[1] * x + M1[5] * y + M1[9] * z + M1[13] * w),
                    z = (M1[2] * x + M1[6] * y + M1[10] * z + M1[14] * w),
