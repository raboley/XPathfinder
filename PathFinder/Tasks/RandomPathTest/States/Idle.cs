...
namespace PathFinder.Tasks.Random.States
{
    ...
    internal class Idle : RandomPathState
    {
        /// <summary>
        ...
        public override void Update()
        {
            try
            {
                if (Character.Navi.Destination(Character.Tasks.RandomPathTask.Options.Posi))
                {
                ...
                if (Character.Navi.DistanceTo(Character.Tasks.RandomPathTask.Options.Posi) > 5)
                {
                    Character.Logger.AddDebugText(TC.rtbDebug, "We are not close to destination so lets keep looking");

                    Character.Tasks.RandomPathTask.Options.KeepMovingStartPosi.X = Character.Api.Player.X;
                    Character.Tasks.RandomPathTask.Options.KeepMovingStartPosi.Y = Character.Api.Player.Y;
                    Character.Tasks.RandomPathTask.Options.KeepMovingStartPosi.Z = Character.Api.Player.Z;
                    Character.FFxiNAV.FindPathToPosi(Character.Tasks.RandomPathTask.Options.KeepMovingStartPosi, Character.Tasks.RandomPathTask.Options.Posi, false);
                }
                else
                    Character.Tasks.RandomPathTask.Stop();
                Character.Tasks.Stop();
